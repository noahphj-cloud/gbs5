...
(function() {
  ...
  const ONLINE_PROFILE_STORAGE_KEY = 'boardgame_online_profile_v2';
  const ONLINE_CLIENT_ID_STORAGE_KEY = 'boardgame_online_client_id';
  const MAX_ONLINE_NAME_LENGTH = 8;
  let onlineClientId = '';
  let onlinePlayerName = '';
  let onlineWinStreak = 0;
  let onlinePlayerNames = Object.create(null);
  let onlinePlayerStreaks = Object.create(null);
  let storedNameNeedsAttention = false;
  const SITE_ACCESS_SESSION_KEY = 'boardgame_site_lock_session';
  const SITE_ACCESS_HEARTBEAT_MS = 5000;
  const SITE_ACCESS_TAB_PROBE_CHANNEL = 'boardgame_site_access_tab_probe_v1';
  let siteAccessAllowed = false;
  let siteAccessBlocked = false;
  let siteAccessHeartbeatTimer = null;
  let siteAccessBootstrapped = false;
  let siteAccessSessionId = '';
  let siteAccessFingerprintPromise = null;
  let siteAccessClaimPromise = null;
  let siteAccessTabRuntimeId = '';
  let siteAccessProbeChannel = null;
  ...
  function generateSiteAccessSessionId() {
    return 'lock_' + Math.random().toString(36).slice(2) + Date.now().toString(36);
  }

  function ensureSiteAccessRuntimeId() {
    if (siteAccessTabRuntimeId) return siteAccessTabRuntimeId;
    siteAccessTabRuntimeId = 'tab_' + Math.random().toString(36).slice(2) + Date.now().toString(36);
    return siteAccessTabRuntimeId;
  }

  function ensureSiteAccessSessionId() {
    if (siteAccessSessionId) return siteAccessSessionId;
    try {
      const stored = window.sessionStorage ? String(window.sessionStorage.getItem(SITE_ACCESS_SESSION_KEY) || '') : '';
      if (/^[A-Za-z0-9_-]{12,128}$/.test(stored)) {
        siteAccessSessionId = stored;
        return siteAccessSessionId;
      }
    } catch (_) {}
    siteAccessSessionId = generateSiteAccessSessionId();
    try { if (window.sessionStorage) window.sessionStorage.setItem(SITE_ACCESS_SESSION_KEY, siteAccessSessionId); } catch (_) {}
    return siteAccessSessionId;
  }

  function createFreshSiteAccessSessionId() {
    siteAccessSessionId = generateSiteAccessSessionId();
    try { if (window.sessionStorage) window.sessionStorage.setItem(SITE_ACCESS_SESSION_KEY, siteAccessSessionId); } catch (_) {}
    return siteAccessSessionId;
  }

  function ensureSiteAccessProbeChannel() {
    if (siteAccessProbeChannel || typeof window.BroadcastChannel !== 'function') return siteAccessProbeChannel;
    try {
      siteAccessProbeChannel = new window.BroadcastChannel(SITE_ACCESS_TAB_PROBE_CHANNEL);
      siteAccessProbeChannel.addEventListener('message', function(ev) {
        const data = ev && ev.data ? ev.data : null;
        if (!data || data.type !== 'probe') return;
        if (data.runtimeId === ensureSiteAccessRuntimeId()) return;
        if (!siteAccessAllowed || siteAccessBlocked) return;
        try {
          siteAccessProbeChannel.postMessage({ type: 'ack', targetRuntimeId: data.runtimeId, runtimeId: ensureSiteAccessRuntimeId() });
        } catch (_) {}
      });
    } catch (_) {
      siteAccessProbeChannel = null;
    }
    return siteAccessProbeChannel;
  }

  function prepareSiteAccessSessionForClaim() {
    const currentId = ensureSiteAccessSessionId();
    const channel = ensureSiteAccessProbeChannel();
    if (!channel) return Promise.resolve(currentId);
    const runtimeId = ensureSiteAccessRuntimeId();
    return new Promise(function(resolve) {
      let settled = false;
      let sawOtherOwner = false;
      let timer = null;
      const finish = function() {
        ...
        if (sawOtherOwner) resolve(createFreshSiteAccessSessionId());
        else resolve(currentId);
      };
      const onMessage = function(ev) {
        const data = ev && ev.data ? ev.data : null;
        if (!data || data.type !== 'ack') return;
        if (data.targetRuntimeId !== runtimeId) return;
        sawOtherOwner = true;
        finish();
      };
      ...
      try {
        channel.postMessage({ type: 'probe', runtimeId: runtimeId, sessionId: currentId });
      } catch (_) {
        finish();
      }
    });
  ...
  function getSiteAccessWebGLSignature() {
    try {
  ...
  function getSiteAccessSignatureSource() {
    ...
    const parts = [
      ...
      'avail=' + [scr.availWidth || 0, scr.availHeight || 0].join('x'),
      'cores=' + (nav.hardwareConcurrency || 0),
      'memory=' + (nav.deviceMemory || 0),
      'touch=' + (nav.maxTouchPoints || 0),
      'gl=' + getSiteAccessWebGLSignature()
    ];
    return parts.join('|');
  }

  function getSiteAccessFingerprint() {
    if (!siteAccessFingerprintPromise) {
      siteAccessFingerprintPromise = sha256Hex(getSiteAccessSignatureSource());
    }
    return siteAccessFingerprintPromise;
  }

  function setSiteAccessBodyState(nextState) {
    const body = document.body;
    if (!body) return;
    body.classList.remove('site-access-pending', 'site-access-blocked', 'site-access-ready');
    body.classList.add(nextState);
  }

  function showSiteAccessGate(mode, message) {
    const gate = document.getElementById('siteAccessGate');
    const kicker = document.getElementById('siteAccessGateKicker');
    const title = document.getElementById('siteAccessGateTitle');
    const text = document.getElementById('siteAccessGateMessage');
    const retryBtn = document.getElementById('siteAccessRetryBtn');
    const safeMode = mode || 'checking';
    if (gate) {
      gate.hidden = false;
      gate.classList.add('show');
      gate.setAttribute('data-mode', safeMode);
    }
    if (retryBtn) retryBtn.hidden = safeMode !== 'blocked';
    if (safeMode === 'blocked') {
      setSiteAccessBodyState('site-access-blocked');
      if (kicker) kicker.textContent = '접속 차단됨';
      if (title) title.textContent = '이 컴퓨터에서는 한 브라우저만 접속할 수 있어';
      if (text) text.textContent = message || '이미 다른 브라우저나 창에서 이 사이트를 사용 중이야. 먼저 그쪽을 닫은 뒤 다시 시도해줘.';
      return;
    }
    setSiteAccessBodyState('site-access-pending');
    if (kicker) kicker.textContent = '접속 확인 중';
    if (title) title.textContent = '이 기기 사용 상태를 확인하고 있어';
    if (text) text.textContent = message || '잠깐만 기다려줘.';
  }

  function hideSiteAccessGate() {
    const gate = document.getElementById('siteAccessGate');
    if (gate) {
      gate.hidden = true;
      gate.classList.remove('show');
    }
    setSiteAccessBodyState('site-access-ready');
  }

  async function postSiteLock(endpoint, options) {
    const fingerprint = await getSiteAccessFingerprint();
    const sessionId = endpoint === 'site-lock-claim'
      ? await prepareSiteAccessSessionForClaim()
      : ensureSiteAccessSessionId();
    return fetch(appUrl(endpoint), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ fingerprint: fingerprint, sessionId: sessionId }),
      keepalive: !!(options && options.keepalive)
    }).then(function(res) {
      return parseJsonResponse(res, 'site lock request failed');
    });
  }

  function stopSiteAccessHeartbeat() {
    if (siteAccessHeartbeatTimer) {
      clearInterval(siteAccessHeartbeatTimer);
      siteAccessHeartbeatTimer = null;
    }
  }

  function releaseSiteAccess(options) {
    stopSiteAccessHeartbeat();
    if (!siteAccessAllowed && !siteAccessBlocked) return;
    siteAccessAllowed = false;
    siteAccessBlocked = false;
    postSiteLock('site-lock-release', options || { keepalive: true }).catch(function() {});
  }

  function blockSiteAccess(message) {
    siteAccessAllowed = false;
    siteAccessBlocked = true;
    stopSiteAccessHeartbeat();
    try { teardownOnlineSession(true, { preserveCurrentGame: false, nextGameMode: null }); } catch (_) {}
    try { hideWaiting(); } catch (_) {}
    showSiteAccessGate('blocked', message);
  }

  function startSiteAccessHeartbeat() {
    stopSiteAccessHeartbeat();
    siteAccessHeartbeatTimer = setInterval(function() {
      if (!siteAccessAllowed || siteAccessBlocked) return;
      postSiteLock('site-lock-heartbeat').catch(function(err) {
        if (err && err.code === 'site_locked_elsewhere') {
          blockSiteAccess(err.message || '다른 브라우저가 이 컴퓨터에서 먼저 사이트를 사용 중이야.');
          return;
        }
      });
    }, SITE_ACCESS_HEARTBEAT_MS);
  }

  function bindSiteAccessLifecycle() {
    if (window.__siteAccessLifecycleBound) return;
    window.__siteAccessLifecycleBound = true;
    // pagehide can fire for bfcache or temporary tab suspension. Releasing the
    // site lock there is too aggressive and can let another browser slip in while
    // this page is still effectively alive. beforeunload is enough here; stale
    // locks are also cleaned server-side by heartbeat TTL.
    window.addEventListener('beforeunload', function() {
      releaseSiteAccess({ keepalive: true });
    });
  }

  function bindSiteAccessRetryButton() {
    const retryBtn = document.getElementById('siteAccessRetryBtn');
    if (!retryBtn || retryBtn.__siteAccessBound) return;
    retryBtn.__siteAccessBound = true;
    retryBtn.addEventListener('click', function() {
      siteAccessBlocked = false;
      siteAccessAllowed = false;
      initializeSiteAccessAndBoot();
    });
  }

  function ensureSiteAccessReadyForAction() {
    if (siteAccessAllowed && !siteAccessBlocked) return true;
    showSiteAccessGate(siteAccessBlocked ? 'blocked' : 'checking', siteAccessBlocked ? undefined : '이 기기 접속 확인이 끝난 뒤에 사용할 수 있어.');
    if (!siteAccessBlocked) initializeSiteAccessAndBoot();
    return false;
  }

  function initializeSiteAccessAndBoot() {
    bindSiteAccessRetryButton();
    if (siteAccessAllowed && !siteAccessBlocked) {
      hideSiteAccessGate();
      if (!siteAccessBootstrapped) {
        siteAccessBootstrapped = true;
        installModeSelectBindings();
      }
      return Promise.resolve(true);
    }
    if (siteAccessClaimPromise) return siteAccessClaimPromise;
    showSiteAccessGate('checking');
    siteAccessClaimPromise = postSiteLock('site-lock-claim')
      .then(function() {
        siteAccessAllowed = true;
        siteAccessBlocked = false;
        hideSiteAccessGate();
        bindSiteAccessLifecycle();
        startSiteAccessHeartbeat();
        if (!siteAccessBootstrapped) {
          siteAccessBootstrapped = true;
          installModeSelectBindings();
        }
      ...
      .catch(function(err) {
        siteAccessAllowed = false;
        siteAccessBlocked = true;
        showSiteAccessGate('blocked', err && err.message ? err.message : undefined);
        return false;
      })
      .finally(function() {
        siteAccessClaimPromise = null;
      });
    return siteAccessClaimPromise;
  }
  ...
  function parseJsonResponse(res, fallbackMessage) {
    return (async function() {
      ...
      if (!res.ok) {
        ...
        err.response = res;
        err.payload = data;
        err.code = data && data.code ? data.code : undefined;
        err.rawText = rawText;
        throw err;
      }
      if (!data) {
  ...
  function sliceNameGraphemes(value, maxLength) {
  ...
  function countNameGraphemes(value) {
    const normalized = String(value || '').normalize('NFC');
    if (typeof Intl !== 'undefined' && Intl && typeof Intl.Segmenter === 'function') {
      const segmenter = new Intl.Segmenter('ko', { granularity: 'grapheme' });
      let count = 0;
      for (const _item of segmenter.segment(normalized)) count += 1;
      return count;
    }
  ...
  function sanitizeOnlinePlayerName(value) {
    const collapsed = String(value || '').normalize('NFC').replace(/\s+/g, ' ').trim();
    return sliceNameGraphemes(collapsed, MAX_ONLINE_NAME_LENGTH);
  }

  function getNameScaleClass(name, streak) {
    const graphemeCount = countNameGraphemes(name);
    const streakDigits = String(sanitizeWinStreak(streak)).length;
    const weight = graphemeCount + streakDigits + (streakDigits ? 4 : 0);
    if (weight >= 13 || graphemeCount >= 8) return 'name-scale-small';
    if (weight >= 10 || graphemeCount >= 6) return 'name-scale-medium';
    return 'name-scale-normal';
  }

  function sanitizeWinStreak(value) {
...
function generateOnlineClientId() {
  try {
    ...
    if (window.crypto && typeof window.crypto.getRandomValues === 'function') {
      ...
      return Array.from(arr).map(function(v) { return v.toString(16).padStart(2, '0'); }).join('');
    }
  } catch (_) {}
  ...
  function getDisplayNameParts(name, streak, options) {
    const opts = options || {};
    const safeName = sanitizeOnlinePlayerName(name || '') || (opts.fallbackName || '플레이어');
    const safeStreak = sanitizeWinStreak(streak);
    const scaleClass = getNameScaleClass(safeName, safeStreak);
    return {
      safeName,
      safeStreak,
      scaleClass,
      text: safeName + ' 🔥(' + safeStreak + ')',
      html: '<span class="name-with-streak ' + scaleClass + '"><span class="name-main">' + escapeHtml(safeName) + '</span><span class="inline-streak">🔥(' + safeStreak + ')</span></span>'
    };
  }

  function persistOnlineProfile() {
    try {
      if (!window.localStorage) return;
      if (!onlineClientId) onlineClientId = generateOnlineClientId();
      localStorage.setItem(ONLINE_NAME_STORAGE_KEY, onlinePlayerName || '');
      localStorage.setItem(ONLINE_CLIENT_ID_STORAGE_KEY, onlineClientId || '');
      localStorage.setItem(ONLINE_PROFILE_STORAGE_KEY, JSON.stringify({ clientId: onlineClientId || '', name: onlinePlayerName || '', streak: sanitizeWinStreak(onlineWinStreak) }));
    } catch (_) {}
  }
  ...
  function loadOnlinePlayerName() {
    ...
    onlinePlayerName = '';
    onlineWinStreak = 0;
    storedNameNeedsAttention = false;
    try {
      if (window.localStorage) {
        const rawProfile = localStorage.getItem(ONLINE_PROFILE_STORAGE_KEY);
        if (rawProfile) {
          try {
            const parsed = JSON.parse(rawProfile);
            onlineClientId = String(parsed && parsed.clientId || '').trim();
            onlinePlayerName = sanitizeOnlinePlayerName(parsed && parsed.name);
            onlineWinStreak = sanitizeWinStreak(parsed && parsed.streak);
          } catch (_) {}
        }
        if (!onlineClientId) onlineClientId = String(localStorage.getItem(ONLINE_CLIENT_ID_STORAGE_KEY) || '').trim();
        if (!onlinePlayerName) {
          onlinePlayerName = sanitizeOnlinePlayerName(localStorage.getItem(ONLINE_NAME_STORAGE_KEY));
        }
      }
    } catch (_) {
      onlineClientId = '';
  ...
  function saveOnlineWinStreak(value) {
    onlineWinStreak = sanitizeWinStreak(value);
    persistOnlineProfile();
    renderOnlineNameUi();
    return onlineWinStreak;
  }

  function syncResolvedSelfOnlineProfile() {
    let changed = false;
    if (typeof playerIndex === 'number') {
      const mappedName = sanitizeOnlinePlayerName(onlinePlayerNames[playerIndex] || '');
      if (mappedName && mappedName !== onlinePlayerName) {
        onlinePlayerName = mappedName;
        changed = true;
      }
      if (Object.prototype.hasOwnProperty.call(onlinePlayerStreaks, playerIndex)) {
        const mappedStreak = sanitizeWinStreak(onlinePlayerStreaks[playerIndex]);
        if (mappedStreak !== onlineWinStreak) {
          onlineWinStreak = mappedStreak;
          changed = true;
        }
      }
    }
    if (changed) persistOnlineProfile();
    return { name: onlinePlayerName || '나', streak: sanitizeWinStreak(onlineWinStreak), changed: changed };
  }

  function applyStoredOnlineProfileFromStorage() {
    // Ignore cross-tab storage updates while this tab owns an online session or queue state.
    if (gameMode === 'online') return false;
    const beforeName = onlinePlayerName;
    const beforeStreak = onlineWinStreak;
    const beforeNeedsAttention = storedNameNeedsAttention;
    loadOnlinePlayerName();
    renderOnlineNameUi();
    return beforeName !== onlinePlayerName || beforeStreak !== onlineWinStreak || beforeNeedsAttention !== storedNameNeedsAttention;
  }
  ...
  function renderOnlineNameUi() {
    ...
    if (editBtn) {
      if (onlinePlayerName) {
        const parts = getDisplayNameParts(onlinePlayerName, onlineWinStreak);
        editBtn.innerHTML = '👤 ' + parts.html;
      } else {
        editBtn.textContent = '👤 이름 설정';
      }
    }
    ...
    if (label) {
      if (onlinePlayerName) {
        const parts = getDisplayNameParts(onlinePlayerName, onlineWinStreak);
        const extra = storedNameNeedsAttention ? ' <span style="opacity:.92;color:#fca5a5;">· 이 이름은 지금 다른 사람이 쓰는 중이라 메인 화면에서 바꿔야 해.</span>' : ' <span style="opacity:.82;">· 메인 화면에서만 변경 가능</span>';
        label.innerHTML = '온라인 이름: ' + parts.html + extra;
      } else {
        label.textContent = '온라인 플레이 전에 이름을 한 번만 설정하면 저장돼. 닉네임 변경은 메인 화면에서만 가능해.';
      }
    }
  }

  function promptOnlinePlayerName(existingName, reasonMessage) {
    const initialValue = sanitizeOnlinePlayerName(existingName || onlinePlayerName || '');
    const guide = reasonMessage ? (reasonMessage + '\n\n') : '';
    const entered = window.prompt(guide + '온라인에서 사용할 이름을 입력해줘.\n- 최대 8글자\n- 한글도 가능', initialValue);
    if (entered === null) return null;
    const normalized = String(entered || '').normalize('NFC').replace(/\s+/g, ' ').trim();
    if (!normalized) {
      alert('이름은 한 글자 이상 입력해줘.');
      return '';
    }
    if (countNameGraphemes(normalized) > MAX_ONLINE_NAME_LENGTH) {
      alert('닉네임은 최대 ' + MAX_ONLINE_NAME_LENGTH + '글자까지 가능해.');
      return '';
    }
    return sanitizeOnlinePlayerName(normalized);
  }
  ...
  function updateOnlinePlayerNames(playersList) {
    ...
    playersList.forEach(function(item) {
      ...
      if (Object.prototype.hasOwnProperty.call(item, 'playerStreak') || Object.prototype.hasOwnProperty.call(item, 'streak')) {
        onlinePlayerStreaks[item.playerIndex] = sanitizeWinStreak(item.playerStreak != null ? item.playerStreak : item.streak);
      }
    });
    if (typeof playerIndex === 'number') {
      const resolved = syncResolvedSelfOnlineProfile();
      if (resolved.changed) renderOnlineNameUi();
    }
  }

  function getKnownPlayerStreak(idx) {
    if (typeof idx === 'number' && Object.prototype.hasOwnProperty.call(onlinePlayerStreaks, idx)) return sanitizeWinStreak(onlinePlayerStreaks[idx]);
    if (typeof idx === 'number' && idx === playerIndex) return sanitizeWinStreak(onlineWinStreak);
    return null;
  }

  function getChatSenderName(msg) {
    const directName = sanitizeOnlinePlayerName(msg && (msg.playerName || msg.name));
    const resolvedIndex = msg && typeof msg.playerIndex === 'number' ? msg.playerIndex : null;
    const resolvedName = directName || (resolvedIndex !== null && onlinePlayerNames[resolvedIndex]) || (resolvedIndex === playerIndex && onlinePlayerName) || (resolvedIndex !== null ? ('플레이어 ' + (resolvedIndex + 1)) : '플레이어');
    const resolvedStreak = sanitizeWinStreak(msg && (msg.playerStreak != null ? msg.playerStreak : msg.streak));
    const streak = (msg && (msg.playerStreak != null || msg.streak != null)) ? resolvedStreak : getKnownPlayerStreak(resolvedIndex);
    return getDisplayNameParts(resolvedName, streak == null ? 0 : streak, { fallbackName: '플레이어' });
  }

  function syncOnlineProfileToServer(options) {
    const opts = options || {};
    const payload = { clientId: onlineClientId || generateOnlineClientId() };
    if (playerId && gameMode === 'online' && opts.includePlayerId !== false) payload.playerId = playerId;
    if (opts.includeName !== false) payload.playerName = onlinePlayerName;
    if (opts.includeStreak !== false) payload.playerStreak = onlineWinStreak;
    return fetch(appUrl('profile'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    }).then(function(res) {
      return parseJsonResponse(res, 'profile sync failed');
    });
  }

  function reserveStoredOnlineProfileSilently() {
    if (!onlinePlayerName || window.location.protocol === 'file:') return Promise.resolve(false);
    return syncOnlineProfileToServer({ includePlayerId: false })
      .then(function(data) {
        storedNameNeedsAttention = false;
        if (data && data.clientId) {
          onlineClientId = String(data.clientId || onlineClientId).trim() || onlineClientId;
          persistOnlineProfile();
        }
        storedNameNeedsAttention = false;
        renderOnlineNameUi();
      ...
      .catch(function(err) {
        if (err && (err.code === 'name_taken' || err.code === 'name_too_long' || err.code === 'name_required' || (err.payload && (err.payload.code === 'name_taken' || err.payload.code === 'name_too_long' || err.payload.code === 'name_required')))) {
          storedNameNeedsAttention = true;
          renderOnlineNameUi();
        }
        return false;
      });
...
function isMenuScreenActive() {
  const page = document.getElementById('pageMenu');
  return !!(page && page.classList.contains('active'));
}

  function editOnlineProfile(reasonMessage) {
    if (!isMenuScreenActive()) {
      showGlobalNotice('닉네임 변경은 메인 화면에서만 가능해.', 'warning', 2600);
      return Promise.resolve(false);
    }
    const previousName = onlinePlayerName || '';
    const previousStreak = onlineWinStreak;
    const picked = promptOnlinePlayerName(previousName, reasonMessage);
    if (picked === null) return Promise.resolve(false);
    if (!picked) return Promise.resolve(false);
    return fetch(appUrl('profile'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ clientId: onlineClientId || generateOnlineClientId(), playerName: picked, playerStreak: previousStreak })
    })
      .then(function(res) {
        return parseJsonResponse(res, 'profile sync failed');
      })
      .then(function(data) {
        ...
        if (data && data.playerName) saveOnlinePlayerName(data.playerName);
        else saveOnlinePlayerName(picked);
        if (data && Object.prototype.hasOwnProperty.call(data, 'playerStreak')) saveOnlineWinStreak(data.playerStreak);
        const changed = previousName && previousName !== onlinePlayerName;
        renderOnlineNameUi();
        showGlobalNotice('온라인 이름을 ' + onlinePlayerName + '로 저장했어.' + (changed ? ' 바로 반영됐어.' : ''), 'info', 2400);
        return true;
      })
      .catch(function(err) {
        console.error(err);
        if (err && (err.code === 'name_taken' || (err.payload && err.payload.code === 'name_taken'))) {
          storedNameNeedsAttention = true;
          renderOnlineNameUi();
          showGlobalNotice('이미 사용 중인 이름이라서 다른 이름이 필요해.', 'warning', 2800);
          return editOnlineProfile('이미 사용 중인 이름이야. 다른 이름을 입력해줘.');
        }
        if (err && (err.code === 'name_too_long' || (err.payload && err.payload.code === 'name_too_long'))) {
          alert((err && err.message) || ('닉네임은 최대 ' + MAX_ONLINE_NAME_LENGTH + '글자까지 가능해.'));
        } else if (err && (err.code === 'name_required' || (err.payload && err.payload.code === 'name_required'))) {
          alert((err && err.message) || '닉네임은 한 글자 이상 입력해줘.');
        } else {
          alert('이름 저장 중 오류가 발생했어. 다시 시도해줘.');
        }
        onlinePlayerName = previousName;
        onlineWinStreak = previousStreak;
        persistOnlineProfile();
        renderOnlineNameUi();
        return false;
      });
  ...
  function buildOnlineVersusMeta(game, playersList) {
    ...
    const leftRole = getRoleInfo(game, 0);
    const rightRole = getRoleInfo(game, 1);
    const leftParts = getDisplayNameParts(
      sanitizeOnlinePlayerName(list[0] && (list[0].playerName || list[0].name)) || leftRole.name,
      sanitizeWinStreak(list[0] && (list[0].playerStreak != null ? list[0].playerStreak : list[0] && list[0].streak)),
      { fallbackName: leftRole.name }
    );
    const rightParts = getDisplayNameParts(
      sanitizeOnlinePlayerName(list[1] && (list[1].playerName || list[1].name)) || rightRole.name,
      sanitizeWinStreak(list[1] && (list[1].playerStreak != null ? list[1].playerStreak : list[1] && list[1].streak)),
      { fallbackName: rightRole.name }
    );
    return {
      ...
      right: {
        name: rightParts.text,
        html: rightParts.html,
        role: rightRole.name,
        sub: '집중하고 첫 수를 노려봐'
      }
    };
  ...
  function showVersusIntro(meta) {
    ...
    if (leftName) {
      if (meta.left.html) leftName.innerHTML = meta.left.html;
      else leftName.textContent = meta.left.name || 'Player 1';
    }
    if (leftRole) leftRole.textContent = meta.left.role || '1P';
    if (leftSub) leftSub.textContent = meta.left.sub || '준비 완료';
    if (rightName) {
      if (meta.right.html) rightName.innerHTML = meta.right.html;
      else rightName.textContent = meta.right.name || 'Player 2';
    }
  ...
  function setupRoleBadge() {
    ...
    badge.style.cssText = 'display:inline-flex;align-items:center;justify-content:center;flex-wrap:wrap;gap:8px;max-width:min(92vw,720px);margin:12px auto 8px;padding:8px 12px;border-radius:999px;background:#111827;border:1px solid #374151;color:#f8fafc;font-weight:700;position:relative;left:50%;transform:translateX(-50%);text-align:center;line-height:1.35;';
    ...
    const selfProfile = syncResolvedSelfOnlineProfile();
    const selfParts = getDisplayNameParts(selfProfile.name || '나', selfProfile.streak, { fallbackName: '나' });
    label.innerHTML = '온라인 · 내 색: ' + escapeHtml(role.name) + ' · ' + selfParts.html;
  ...
  function teardownOnlineSession(notifyServer, options) {
    ...
    if (joinRequestInFlight && !playerId) cancelledJoinSessions[onlineSessionToken] = true;
    ...
    onlinePlayerStreaks = Object.create(null);
  ...
  function startOffline() {
    try {
      if (!ensureSiteAccessReadyForAction()) return;
  ...
  function installModeSelectBindings() {
    ...
    loadOnlinePlayerName();
    renderOnlineNameUi();
    reserveStoredOnlineProfileSilently();
    if (!window.__onlineProfileStorageBound) {
      window.__onlineProfileStorageBound = true;
      window.addEventListener('storage', function(ev) {
        if (!ev || !ev.key) return;
        if (ev.key !== ONLINE_PROFILE_STORAGE_KEY && ev.key !== ONLINE_NAME_STORAGE_KEY && ev.key !== ONLINE_CLIENT_ID_STORAGE_KEY) return;
        applyStoredOnlineProfileFromStorage();
      });
    }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeSiteAccessAndBoot, { once: true });
  } else {
    initializeSiteAccessAndBoot();
  }
  ...
  function joinOnlineGame(gameType, requestedPlayerName) {
    ...
    fetch(appUrl('join'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ gameType, clientId: onlineClientId, playerName: ensuredName, playerStreak: onlineWinStreak })
    })
      ...
      .catch(err => {
        ...
        if (err && (err.code === 'client_already_online' || (err.payload && err.payload.code === 'client_already_online'))) {
          alert('같은 브라우저에서는 한 창만 온라인 플레이할 수 있어. 이미 다른 창에서 온라인 중이야.');
          return;
        }
        if (err && (err.code === 'name_change_only_in_menu' || (err.payload && err.payload.code === 'name_change_only_in_menu'))) {
          alert((err && err.message) || '같은 브라우저에서 이미 온라인 플레이 중이라서 닉네임을 지금 바꿀 수 없어.');
          return;
        }
        if (err && (err.code === 'name_taken' || (err.payload && err.payload.code === 'name_taken'))) {
          storedNameNeedsAttention = true;
          renderOnlineNameUi();
          showGlobalNotice('지금 이름은 다른 사람이 쓰고 있어서 다시 정해야 해.', 'warning', 3000);
          editOnlineProfile('이미 사용 중인 이름이야. 다른 이름을 입력해줘.');
          return;
        }
        if (err && (err.code === 'name_too_long' || (err.payload && err.payload.code === 'name_too_long'))) {
          alert((err && err.message) || ('닉네임은 최대 ' + MAX_ONLINE_NAME_LENGTH + '글자까지 가능해.'));
        ...
        if (err && (err.code === 'name_required' || (err.payload && err.payload.code === 'name_required'))) {
          alert((err && err.message) || '온라인 플레이 전에 닉네임을 먼저 설정해줘.');
          return;
        }
  ...
  function getSnapshotWinnerInfo(snapshot) {
    const state = snapshot && snapshot.state ? snapshot.state : null;
    const gameState = state && currentGame ? state[currentGame] : null;
    if (!gameState) return null;
    if (gameState.draw) return { outcome: 'draw', winner: 0 };
    if (typeof gameState.winner === 'number' && gameState.winner > 0) {
      return { outcome: 'win', winner: gameState.winner };
    }
    return null;
  }

  function applySavedWinStreakFromOutcome(payload) {
    const snapshotInfo = getSnapshotWinnerInfo(payload && payload.snapshot);
    if (snapshotInfo) {
      if (snapshotInfo.outcome === 'draw') {
        saveOnlineWinStreak(0);
        return '무승부라서 연승은 🔥(0)으로 리셋됐어.';
      }
      if (typeof playerIndex === 'number' && snapshotInfo.winner === playerIndex + 1) {
        const nextStreak = saveOnlineWinStreak(onlineWinStreak + 1);
        return '연승 갱신! ' + getDisplayNameParts(onlinePlayerName || '나', nextStreak, { fallbackName: '나' }).text;
      }
      if (typeof playerIndex === 'number' && snapshotInfo.winner !== playerIndex + 1) {
        saveOnlineWinStreak(0);
        return '패배로 연승은 🔥(0)으로 리셋됐어.';
      }
      return '';
    }

    const endReason = payload && payload.reason;
    if (endReason === 'self_left') {
      saveOnlineWinStreak(0);
      return '대국을 중간에 나가서 연승은 🔥(0)으로 리셋됐어.';
    }
    return '';
  }
  ...
  function finalizeOnlineGame(payload) {
    ...
    const streakMessage = applySavedWinStreakFromOutcome(finalPayload);
    ...
    try {
      if (preservedGame && gameConfigs[preservedGame] && gameConfigs[preservedGame].onMoveCallbackName) {
        window[gameConfigs[preservedGame].onMoveCallbackName] = null;
      }
    } catch (err) {
      console.warn('online move callback cleanup failed after finalize', err);
    }
    try {
      if (typeof window.blokusSetOnlineContext === 'function') window.blokusSetOnlineContext(false, null);
    } catch (err) {
      console.warn('blokus online context reset failed after finalize', err);
    }
    try {
      if (typeof window.orbitoSetOnlineTurnState === 'function') window.orbitoSetOnlineTurnState(true, false, false);
    } catch (err) {
      console.warn('orbito online state reset failed after finalize', err);
    }
    roomId = null;
    playerIndex = null;
    playerId = null;
    ...
    if (keepResultScreen) {
      ...
      refreshOfflineControls(preservedGame);
    } else {
      currentGame = null;
    ...
    let visibleMessage = keepResultScreen ? (endMessage + ' 결과 화면은 그대로 둘게.') : endMessage;
    if (streakMessage) visibleMessage += ' ' + streakMessage;
    updateTurnControl();
    showGlobalNotice(visibleMessage, tone, 4600);
  }
  ...
  function setupEventSource() {
    ...
    eventSource.addEventListener('start', function(event) {
      try {
        ...
        if (data && Object.prototype.hasOwnProperty.call(data, 'playerStreak')) {
          onlineWinStreak = sanitizeWinStreak(data.playerStreak);
          persistOnlineProfile();
          renderOnlineNameUi();
    ...
    eventSource.addEventListener('chat', function(event) {
      try {
        ...
        if (data && typeof data.playerIndex === 'number' && (data.playerStreak != null || data.streak != null)) {
          onlinePlayerStreaks[data.playerIndex] = sanitizeWinStreak(data.playerStreak != null ? data.playerStreak : data.streak);
        }
    ...
    eventSource.addEventListener('profile', function(event) {
      try {
        const data = JSON.parse(event.data);
        if (!isCurrentOnlineSession(sessionToken) || !isMatchingRoomEvent(data, false)) return;
        clearWaitingRecovery(false);
        if (data && typeof data.playerIndex === 'number') {
          if (data.playerName) onlinePlayerNames[data.playerIndex] = sanitizeOnlinePlayerName(data.playerName) || onlinePlayerNames[data.playerIndex];
          if (data.playerStreak != null) onlinePlayerStreaks[data.playerIndex] = sanitizeWinStreak(data.playerStreak);
        }
        if (data && data.playerId && data.playerId === playerId) {
          if (data.playerName) onlinePlayerName = sanitizeOnlinePlayerName(data.playerName) || onlinePlayerName;
          if (data.playerStreak != null) onlineWinStreak = sanitizeWinStreak(data.playerStreak);
          persistOnlineProfile();
          renderOnlineNameUi();
          try { setupRoleBadge(); } catch (_) {}
        }
      } catch (e) {
        console.error(e);
      }
  ...
  function beginGame(startPayload) {
    ...
    syncResolvedSelfOnlineProfile();
    setupRoleBadge();
    setupChatUI();
    showVersusIntro(buildOnlineVersusMeta(currentGame, startPayload && startPayload.players));
    appendSystemMessage(myTurnFlag ? '매칭 완료! 네가 선공이야.' : '매칭 완료! 상대가 먼저 둬.');
  }
  ...
  function handleRemoteMove(eventData) {
    ...
    try {
      if ((kind === 'axis4' || currentGame === 'axis4') && Array.isArray(payload)) {
        ...
        if (typeof window[cfg.apply] === 'function') {
          const result = window[cfg.apply](payload);
          appliedMeta = result || { applied: false, gameOver: false, keepTurn: false };
        }
  ...
  function setupChatUI() {
    ...
    chat.innerHTML =
      ...
      '<input id="chatInput" type="text" maxlength="300" placeholder="메시지 입력..." />' +
    ...
    function sendChatMessage() {
      ...
      fetch(appUrl('chat'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ playerId, message: text, playerName: onlinePlayerName, playerStreak: onlineWinStreak })
      })
  ...
  function appendChatMessage(msg) {
    ...
    const senderParts = getChatSenderName(msg);
    if (msg && typeof msg.playerIndex === 'number' && msg.playerIndex === playerIndex) {
      p.style.color = '#c4b5fd';
    }
    p.innerHTML = senderParts.html + ': ' + escapeHtml(msg && msg.message ? msg.message : '');
    list.appendChild(p);
