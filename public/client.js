...
(function() {
  ...
  const ONLINE_RESULT_OVERLAY_IDS = ['axis4Overlay', 'gOverlay', 'dabOverlay', 'blokusOverlay', 'quorOverlay', 'halmaOverlay', 'orbitoWinBanner'];
  ...
  function syncOnlineResultStatus(snapshot) {
    ...
    if (gameState.draw) {
      statusEl.textContent = '무승부야';
      return;
    }
    const winnerLabel = getOnlineWinnerLabel(gameState);
    statusEl.textContent = winnerLabel ? (winnerLabel + '가 이겼어') : '게임이 끝났어';
  }
  ...
  function finalizeOnlineGame(payload) {
    ...
    const visibleMessage = keepResultScreen ? (endMessage + ' 결과 화면은 그대로 둘게.') : endMessage;
    updateTurnControl();
  ...
  function applySnapshot(snapshot, options) {
    ...
    syncCurrentGameHeaderUi();
    if (isOnlineGameViewActive()) hideOnlineResultOverlays();
