...
(function() {
  const SUPPORTED_GAMES = ['axis4', 'gomoku', 'quoridor', 'dab', 'orbito', 'halma'];
  ...
  const queueGameNames = { axis4: '축고정 사목', gomoku: '오목', quoridor: '콰리도', dab: '점과 상자', orbito: '오비토', halma: '별모양 점프 게임' };
  const ruleContent = {
    gomoku: {
      kicker: '오목 · 기본 규칙',
      title: '오목 하는 법',
      subtitle: '빈 칸에 돌을 놓아 같은 색 돌 5개를 가로, 세로, 대각선 중 하나로 먼저 이어 만들면 이겨.',
      sections: [
        { title: '진행 방법', bullets: ['흑과 백이 번갈아 빈 칸에 돌을 하나씩 둬.', '이미 돌이 있는 칸에는 둘 수 없어.', '다시하기 전까지 돌은 움직이지 않아.'] },
        { title: '승리 조건', bullets: ['내 돌 5개를 한 줄로 먼저 만들면 승리야.', '가로, 세로, 대각선 어느 방향이든 가능해.'] }
      ],
      tip: '중앙을 먼저 잡고, 공격 줄과 방어 줄을 동시에 보는 게 좋아.'
    },
    dab: {
      kicker: '점과 상자 · 기본 규칙',
      title: '점과 상자 하는 법',
    ...
    orbito: {
      kicker: '오비토 · 기본 규칙',
      title: '오비토 하는 법',
      subtitle: '상대 돌을 한 번 밀고, 내 돌을 하나 둔 뒤, 판의 궤도를 회전시켜 4개를 연결하면 이겨.',
      sections: [
        { title: '한 턴의 순서', bullets: ['상대 돌 하나를 인접한 빈칸으로 움직이거나, 못 움직이면 건너뛰어.', '그 다음 내 돌 하나를 빈칸에 둬.', '마지막으로 판을 회전시켜 돌 위치를 바꿔.'] },
    ...
    blokus: {
      kicker: '브로커스 스타일 · 기본 규칙', title: '브로커스 스타일 하는 법', subtitle: '조각을 꼭짓점끼리만 닿게 놓으면서 더 많은 칸을 차지하면 유리해.',
      sections: [{ title: '핵심 규칙', bullets: ['내 조각끼리는 변으로 닿으면 안 되고 꼭짓점으로만 닿아야 해.', '첫 조각은 시작점에서 시작해야 해.', '더 이상 둘 수 없으면 패스를 고려해야 해.'] }],
  ...
  function installInfoButtons() {
    var headerMaps = {
      gomoku: 'pageGomoku', dab: 'pageDAB', quoridor: 'pageQuoridor', orbito: 'pageOrbito', axis4: 'pageAxis4', halma: 'pageHalma', blokus: 'pageBlokus', airhockey: 'pageAirHockey', game2048: 'page2048'
    };
    Object.keys(headerMaps).forEach(function(key) {
      var page = document.getElementById(headerMaps[key]);
      if (!page) return;
      var header = page.querySelector('.game-header');
      if (!header || header.querySelector('.info-btn')) return;
      var restartBtn = header.querySelector('.restart-btn');
      var actionsWrap = document.createElement('div');
      actionsWrap.className = 'game-header-actions';
      var infoBtn = document.createElement('button');
      infoBtn.className = 'info-btn';
      infoBtn.type = 'button';
      infoBtn.setAttribute('aria-label', '게임 규칙 보기');
      infoBtn.textContent = 'i';
      bindTapFriendlyAction(infoBtn, function(ev) {
        if (ev) ev.stopPropagation();
        openRuleModal(key);
      });
      if (restartBtn) {
  ...
  if (typeof originalShowPage === 'function') {
    window.showPage = function(id) {
      ...
      if (id === 'pageMenu') fetchQueueStatus(true);
      return result;
    };
  }
  ...
  function getOnlineUnsafeSelectors() {
    const map = {
      ...
      halma: ['#pageHalma .restart-btn', '#pageHalma button[onclick="halmaRestart()"]', '#pageHalma button[onclick="halmaSetMode(2)"]', '#pageHalma button[onclick="halmaSetMode(3)"]', '#pageHalma button[onclick="halmaClearSelection()"]', '#pageHalma button[onclick="halmaEndTurn()"]']
    };
  ...
  function getSnapshotEndMessage(snapshot) {
    ...
    if ((currentGame === 'axis4' || currentGame === 'gomoku' || currentGame === 'dab' || currentGame === 'orbito' || currentGame === 'halma') && typeof gameState.winner === 'number') {
      if (gameState.winner === 1) return '빨강 승리로 게임이 종료되었어.';
      if (gameState.winner === 2) return '파랑 승리로 게임이 종료되었어.';
  ...
  function lockDisconnectedMatch(reasonText) {
    ...
    if (gameMode === 'online' && onlineGameStarted) {
      ...
      disconnectRecoveryTimer = setTimeout(function() {
        ...
        teardownOnlineSession(false);
        gameMode = null;
        alert('온라인 연결 복구가 오래 걸려서 대국을 종료했어. 다시 온라인 플레이를 눌러줘.');
      }, 12000);
    }
  }
        ...
        sendMove(currentGame, payload)
          .then((data) => {
            ...
            if (isOwnOrbitoRotate && data && data.snapshot && data.snapshot.state && data.snapshot.state.orbito && typeof window.orbitoAnimateToState === 'function') {
              ...
              if (snapshot.roomId && roomId && snapshot.roomId !== roomId) return;
              const revision = typeof snapshot.revision === 'number' ? snapshot.revision : null;
              if (revision !== null && revision > lastSnapshotRevision) lastSnapshotRevision = revision;
              if (snapshot.roomId && !roomId) roomId = snapshot.roomId;
              if (snapshot.gameOver === true) myTurnFlag = false;
  ...
  function handleRemoteMove(eventData) {
    ...
    if (eventData.gameOver === true || appliedMeta.gameOver === true) {
      myTurnFlag = false;
    } else if (appliedMeta.keepTurn === true || eventData.keepTurn === true) {
      myTurnFlag = true;
