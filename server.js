...
const AIR_SPIN_FRICTION = 0.972;
const AIR_MAX_PUCK_SPEED = 18;
const AIR_MAX_PUCK_SPIN = 0.45;
const AIR_PUCK_RESTITUTION = 0.88;
const AIR_WALL_RESTITUTION = 0.94;
const AIR_WALL_TANGENT_DAMPING = 0.985;
const AIR_WALL_SPIN_TRANSFER = 0.0015;
const AIR_PADDLE_TRANSFER = 0.25;
const AIR_TANGENT_DAMPING = 0.34;
const AIR_TANGENT_TRANSFER = 0.08;
const AIR_SPIN_TRANSFER = 0.004;
const AIR_SPIN_TO_TANGENT = 0.0;
const AIR_CONTACT_MARGIN = 1.2;
...
function airAddSpinFromPaddle(puck, puckTangent, paddleTangent) {
  // 접선 방향 상대속도는 퍽의 회전으로만 일부 넘긴다.
  // 회전값을 다시 진행 방향에 강하게 섞으면 퍽이 패들 둘레를 따라 도는 '궤도' 현상이 생긴다.
  const relativeSurface = paddleTangent - puckTangent;
  const deltaOmega = Math.max(-0.10, Math.min(0.10, relativeSurface * AIR_SPIN_TRANSFER));
...
function airApplyWallSpin(state, wall, tangentVelocity) {
  ...
  puck.omega = (puck.omega || 0) + Math.max(-0.08, Math.min(0.08, sign * tangent * AIR_WALL_SPIN_TRANSFER));
  airClampPuckSpin(puck);
}
...
function airCollidePaddle(state, p) {
  ...
  // 이미 서로 멀어지거나 거의 스치는 정도면 속도는 건드리지 않고 위치만 접촉선 밖으로 보정한다.
  // 약한 접선 접촉까지 반사하면 퍽이 패들 둘레를 타고 도는 느낌이 생긴다.
  if (relativeNormal >= -0.08) return;
  ...
  if (paddleNormal > 0) {
    outNormal += Math.min(2.4, paddleNormal * AIR_PADDLE_TRANSFER);
    outNormal = Math.max(outNormal, Math.min(AIR_MIN_SHOT_SPEED, Math.max(2.4, paddleNormal + 1.4)));
  }
  airAddSpinFromPaddle(puck, puckTangent, paddleTangent);
  let outTangent = puckTangent * AIR_TANGENT_DAMPING + paddleTangent * AIR_TANGENT_TRANSFER;
  // 접선 속도가 법선 반발보다 너무 크면 퍽이 패들 표면을 타고 빙글 도는 것처럼 보인다.
  // 충돌 직후에는 항상 충분히 바깥으로 빠져나가게 법선 성분을 우선한다.
  const minExitNormal = Math.min(3.6, Math.max(1.35, Math.abs(relativeNormal) * 0.65));
  outNormal = Math.max(outNormal, minExitNormal);
  const tangentLimit = Math.max(1.1, Math.abs(outNormal) * 0.55);
  if (outTangent > tangentLimit) outTangent = tangentLimit;
  if (outTangent < -tangentLimit) outTangent = -tangentLimit;
