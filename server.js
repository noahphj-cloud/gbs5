...
function applyOrbitoMove(state, payload, playerIndex) {
  ...
  if (state.stage === 'move') {
    ...
    if (payload.rotateOnly) return { ok: false, status: 400, error: 'rotate-only payload not allowed in move stage' };
    if (payload.moveEnemy && payload.skippedEnemy) return { ok: false, status: 400, error: 'enemy move and skip cannot both be set' };
    if (payload.moveEnemy && typeof payload.moveEnemy === 'object') {
      ...
      if (orbitoPlayerHasAnyLegalMove(board, enemy)) return { ok: false, status: 409, error: 'enemy skip not allowed while legal move exists' };
    } else {
  ...
  if (lines.red && lines.blue) {
    ...
    if (state.extraRotationsLeft === 0) state.extraRotationsLeft = 4;
...
function routeMatches(pathname, endpoint) {
  ...
  return pathname.length > endpoint.length && pathname.endsWith(endpoint) && pathname[pathname.length - endpoint.length - 1] === '/';
}
