...
function applyOrbitoMove(state, payload, playerIndex) {
  ...
  const startedInRotateStage = state.stage === 'rotate';
  if (state.stage === 'move') {
    const enemy = player === 1 ? 2 : 1;
    if (payload.moveEnemy && typeof payload.moveEnemy === 'object') {
      ...
      if (!orbitoHasAdjacentEmpty(board, fx, fy)) return { ok: false, status: 409, error: 'enemy has no legal move' };
      board[fy][fx] = 0;
      board[ty][tx] = enemy;
    } else if (!payload.skippedEnemy) {
      return { ok: false, status: 400, error: 'enemy move or skip required' };
    }
    ...
    board[py][px] = player;
    reserves[player] -= 1;
  } else if (state.stage === 'rotate') {
    if (!payload.rotateOnly) return { ok: false, status: 400, error: 'rotate-only payload required' };
    if (payload.moveEnemy || payload.place || payload.skippedEnemy) return { ok: false, status: 400, error: 'rotate-only payload must not include move or place' };
  } else {
    return { ok: false, status: 409, error: 'invalid orbito stage' };
  }
  ...
  if (lines.red && lines.blue) {
    ...
    else state.extraRotationsLeft -= 1;
  ...
  const isRotateStage = startedInRotateStage;
  return {
    ok: true,
    authoritativeMove: { kind: 'orbito', payload: {
      player,
      moveEnemy: isRotateStage ? null : (payload.moveEnemy ? { from: { x: Number(payload.moveEnemy.from.x), y: Number(payload.moveEnemy.from.y) }, to: { x: Number(payload.moveEnemy.to.x), y: Number(payload.moveEnemy.to.y) } } : null),
      skippedEnemy: isRotateStage ? false : !!payload.skippedEnemy,
      place: isRotateStage ? null : (payload.place ? { x: Number(payload.place.x), y: Number(payload.place.y) } : null),
      rotateOnly: !!payload.rotateOnly
    } },
    keepTurn: false,
    gameOver
  };
...
function quoridorWallLegal(state, playerIndex, x, y, ori) {
  ...
  if (ori === 'h') {
    ...
    const ok = quoridorCanReachGoal(state, state.pawns[0].x, state.pawns[0].y, state.pawns[0].goal) && quoridorCanReachGoal(state, state.pawns[1].x, state.pawns[1].y, state.pawns[1].goal);
  ...
  try {
    ...
    while (queue.length > 0) {
      ...
      rooms[roomId] = {
        ...
        turnIndex: 0,
        revision: 0,
        state: gameType === 'dab' ? { dab: createDabState() } : gameType === 'axis4' ? { axis4: createAxis4State() } : gameType === 'gomoku' ? { gomoku: createGomokuState() } : gameType === 'quoridor' ? { quoridor: createQuoridorState() } : gameType === 'orbito' ? { orbito: createOrbitoState() } : {}
      };
...
async function handleMove(req, res) {
  try {
    ...
    const expectedKindByGame = { dab: 'dab', axis4: 'axis4', gomoku: 'gomoku', quoridor: 'quoridor', orbito: 'orbito' };
...
function serveStatic(req, res) {
  ...
  const safeRequestPath = urlPath.replace(/^\/+/, '');

  function resolveStaticCandidate(relPath) {
    const candidatePath = path.resolve(publicRoot, relPath);
    const relativePath = path.relative(publicRoot, candidatePath);
    if (relativePath.startsWith('..') || path.isAbsolute(relativePath)) return null;
    return candidatePath;
  }

  const staticCandidates = [];
  let candidateRel = safeRequestPath;
  while (candidateRel) {
    staticCandidates.push(candidateRel);
    const slashIndex = candidateRel.indexOf('/');
    if (slashIndex === -1) break;
    candidateRel = candidateRel.slice(slashIndex + 1);
  }
  if (!staticCandidates.length) staticCandidates.push('index.html');

  let filePath = null;
  for (const relPath of staticCandidates) {
    const resolved = resolveStaticCandidate(relPath);
    if (resolved) {
      filePath = resolved;
      break;
    }
  ...
  function trySendStatic(candidates, index) {
    ...
    fs.stat(candidates[index], (err, stat) => {
      if (!err && stat.isFile()) {
        sendFile(candidates[index]);
        return;
      }
      trySendStatic(candidates, index + 1);
    });
  }

  const resolvedCandidates = staticCandidates
    .map(resolveStaticCandidate)
    .filter(Boolean);
  trySendStatic(resolvedCandidates, 0);
}
