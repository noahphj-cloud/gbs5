  ...
  try {
    ...
    while (queue.length > 0) {
      ...
      rooms[roomId] = {
        ...
        turnIndex: 0,
        revision: 0,
        state: gameType === 'dab' ? { dab: createDabState() } : gameType === 'axis4' ? { axis4: createAxis4State() } : gameType === 'gomoku' ? { gomoku: createGomokuState() } : gameType === 'quoridor' ? { quoridor: createQuoridorState() } : {}
      };
...
async function handleMove(req, res) {
  try {
    ...
    const expectedKindByGame = { dab: 'dab', axis4: 'axis4', gomoku: 'gomoku', quoridor: 'quoridor' };
