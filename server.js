...
const rooms = {};
const players = {};
const nameClaimsByClientId = Object.create(null);
const nameClaimIndex = Object.create(null);
const siteLocksByFingerprint = Object.create(null);
...
const API_ENDPOINTS = ['join', 'events', 'state', 'queue-status', 'move', 'chat', 'leave', 'profile', 'site-lock-claim', 'site-lock-heartbeat', 'site-lock-release'];

const MAX_PLAYER_NAME_LENGTH = 8;
const NAME_CLAIM_TTL_MS = 90 * 24 * 60 * 60 * 1000;
const NAME_CLAIMS_FILE = path.join(__dirname, 'name_claims.json');
const SITE_LOCK_TTL_MS = 20 * 1000;
const QUEUED_PLAYER_CONNECT_GRACE_MS = 3000;
...
function countGraphemes(value) {
  ...
  if (typeof Intl !== 'undefined' && Intl && typeof Intl.Segmenter === 'function') {
    const segmenter = new Intl.Segmenter('ko', { granularity: 'grapheme' });
    let count = 0;
    for (const _part of segmenter.segment(normalized)) count += 1;
    return count;
  }
  return Array.from(normalized).length;
}

function collapsePlayerName(value) {
  return String(value || '').normalize('NFC').replace(/\s+/g, ' ').trim();
}

function validateRequestedPlayerName(value) {
  const collapsed = collapsePlayerName(value);
  if (!collapsed) {
    const err = new Error('닉네임은 한 글자 이상 입력해줘.');
    err.status = 400;
    err.code = 'name_required';
    throw err;
  }
  if (countGraphemes(collapsed) > MAX_PLAYER_NAME_LENGTH) {
    const err = new Error('닉네임은 최대 ' + MAX_PLAYER_NAME_LENGTH + '글자까지 가능해.');
    err.status = 400;
...
function normalizePlayerName(value) {
  const collapsed = collapsePlayerName(value);
  const cleaned = sliceGraphemes(collapsed, MAX_PLAYER_NAME_LENGTH);
  return cleaned || '플레이어';
}

function canonicalizePlayerName(value) {
  return normalizePlayerName(value).toLocaleLowerCase('ko-KR');
}
...
function rebuildNameClaimIndex() {
  Object.keys(nameClaimIndex).forEach((key) => delete nameClaimIndex[key]);
  Object.keys(nameClaimsByClientId).forEach((clientId) => {
    const claim = nameClaimsByClientId[clientId];
    if (!claim || !claim.canonicalName || !claim.normalizedName) return;
    nameClaimIndex[claim.canonicalName] = clientId;
  });
}

function pruneExpiredNameClaims() {
  const cutoff = now() - NAME_CLAIM_TTL_MS;
  let changed = false;
  Object.keys(nameClaimsByClientId).forEach((clientId) => {
    const claim = nameClaimsByClientId[clientId];
    if (!claim) {
      delete nameClaimsByClientId[clientId];
      changed = true;
      return;
    }
  ...
  if (changed) rebuildNameClaimIndex();
  return changed;
}

function persistNameClaims() {
  try {
    fs.writeFileSync(NAME_CLAIMS_FILE, JSON.stringify(nameClaimsByClientId, null, 2));
  } catch (err) {
    console.warn('failed to persist name claims', err && err.message ? err.message : err);
  }
}

function loadNameClaims() {
  try {
    if (!fs.existsSync(NAME_CLAIMS_FILE)) return;
    const parsed = JSON.parse(fs.readFileSync(NAME_CLAIMS_FILE, 'utf8'));
    if (!parsed || typeof parsed !== 'object') return;
    Object.keys(parsed).forEach((clientId) => {
      const safeClientId = normalizeClientId(clientId);
      const claim = parsed[clientId];
      if (!safeClientId || !claim || typeof claim !== 'object') return;
      const normalizedName = normalizePlayerName(claim.normalizedName || claim.playerName || claim.name || '');
      const canonicalName = canonicalizePlayerName(normalizedName);
      nameClaimsByClientId[safeClientId] = {
        clientId: safeClientId,
        normalizedName,
        canonicalName,
        updatedAt: Number(claim.updatedAt) || now()
      };
    });
    rebuildNameClaimIndex();
    if (pruneExpiredNameClaims()) persistNameClaims();
  } catch (err) {
    console.warn('failed to load name claims', err && err.message ? err.message : err);
  }
}

function getClientNameClaim(clientId) {
  const safeClientId = normalizeClientId(clientId);
  if (!safeClientId) return null;
  return nameClaimsByClientId[safeClientId] || null;
}

function getReservedPlayerNameForClient(clientId) {
  const claim = getClientNameClaim(clientId);
  return claim && claim.normalizedName ? claim.normalizedName : '';
}

function isPlayerNameActive(player) {
  return !!(player && (player.status === 'queued' || player.status === 'matched'));
}

function findActivePlayerByName(name, excludePlayerId) {
  const target = canonicalizePlayerName(name);
  ...
  for (let i = 0; i < ids.length; i += 1) {
    ...
    if (!isPlayerNameActive(player)) continue;
    if (canonicalizePlayerName(player.playerName) === target) return player;
  }
  return null;
}

function findActivePlayerByClientId(clientId, excludePlayerId) {
  const safeClientId = normalizeClientId(clientId);
  if (!safeClientId) return null;
  const ids = Object.keys(players);
  for (let i = 0; i < ids.length; i += 1) {
    ...
    const player = players[pid];
    if (!isPlayerNameActive(player)) continue;
    if (normalizeClientId(player.clientId) === safeClientId) return player;
  }
  return null;
}

function reservePlayerNameForClient(name, clientId, excludePlayerId) {
  pruneExpiredNameClaims();
  const normalized = normalizePlayerName(name);
  const canonical = canonicalizePlayerName(normalized);
  const duplicate = findActivePlayerByName(normalized, excludePlayerId);
  if (duplicate) {
    const err = new Error('이미 사용 중인 이름이야. 다른 이름을 입력해줘.');
    err.status = 409;
    err.code = 'name_taken';
    throw err;
  }

  const claimedByClientId = nameClaimIndex[canonical];
  const safeClientId = normalizeClientId(clientId);
  if (claimedByClientId && claimedByClientId !== safeClientId) {
    const err = new Error('이미 사용 중인 이름이야. 다른 이름을 입력해줘.');
    err.status = 409;
    err.code = 'name_taken';
    throw err;
  ...
  if (safeClientId) {
    const currentClaim = getClientNameClaim(safeClientId);
    if (currentClaim && currentClaim.canonicalName && currentClaim.canonicalName !== canonical) {
      delete nameClaimIndex[currentClaim.canonicalName];
    }
    nameClaimsByClientId[safeClientId] = {
      clientId: safeClientId,
    ...
    nameClaimIndex[canonical] = safeClientId;
    persistNameClaims();
  }
...
function normalizeSiteLockFingerprint(value) {
  const raw = String(value || '').trim().toLowerCase();
  return /^[a-f0-9]{24,128}$/.test(raw) ? raw : '';
}

function normalizeSiteLockSessionId(value) {
  const raw = String(value || '').trim();
  return /^[A-Za-z0-9_-]{12,128}$/.test(raw) ? raw : '';
}

function pruneExpiredSiteLocks() {
  const cutoff = now() - SITE_LOCK_TTL_MS;
  Object.keys(siteLocksByFingerprint).forEach((fingerprint) => {
    const lock = siteLocksByFingerprint[fingerprint];
    if (!lock) {
      delete siteLocksByFingerprint[fingerprint];
      return;
    }
    const lastSeen = Number(lock.lastSeenAt) || 0;
    if (!lastSeen || lastSeen < cutoff) delete siteLocksByFingerprint[fingerprint];
  });
}

function getActiveSiteLock(fingerprint) {
  pruneExpiredSiteLocks();
  const safeFingerprint = normalizeSiteLockFingerprint(fingerprint);
  if (!safeFingerprint) return null;
  return siteLocksByFingerprint[safeFingerprint] || null;
}

function claimSiteLock(fingerprint, sessionId, meta) {
  const safeFingerprint = normalizeSiteLockFingerprint(fingerprint);
  const safeSessionId = normalizeSiteLockSessionId(sessionId);
  if (!safeFingerprint || !safeSessionId) {
    const err = new Error('잘못된 사이트 잠금 정보야.');
    err.status = 400;
    err.code = 'invalid_site_lock';
    throw err;
  }
  pruneExpiredSiteLocks();
  const existing = siteLocksByFingerprint[safeFingerprint];
  if (existing && existing.sessionId !== safeSessionId) {
    const err = new Error('같은 컴퓨터에서는 한 브라우저만 이 사이트를 열 수 있어. 다른 창이나 브라우저를 먼저 닫아줘.');
    err.status = 409;
    err.code = 'site_locked_elsewhere';
    throw err;
  }
  const stamp = now();
  siteLocksByFingerprint[safeFingerprint] = {
    fingerprint: safeFingerprint,
    sessionId: safeSessionId,
    claimedAt: existing && existing.sessionId === safeSessionId ? (existing.claimedAt || stamp) : stamp,
    lastSeenAt: stamp,
    userAgent: meta && meta.userAgent ? String(meta.userAgent).slice(0, 300) : '',
    remoteAddress: meta && meta.remoteAddress ? String(meta.remoteAddress).slice(0, 120) : ''
  };
  return siteLocksByFingerprint[safeFingerprint];
}

function refreshSiteLock(fingerprint, sessionId, meta) {
  return claimSiteLock(fingerprint, sessionId, meta);
}

function releaseSiteLock(fingerprint, sessionId) {
  const safeFingerprint = normalizeSiteLockFingerprint(fingerprint);
  const safeSessionId = normalizeSiteLockSessionId(sessionId);
  if (!safeFingerprint || !safeSessionId) return false;
  const existing = siteLocksByFingerprint[safeFingerprint];
  if (existing && existing.sessionId === safeSessionId) {
    delete siteLocksByFingerprint[safeFingerprint];
    return true;
  }
  return false;
}

async function handleSiteLockClaim(req, res) {
  try {
    const body = await readJsonBody(req);
    const lock = claimSiteLock(body && body.fingerprint, body && body.sessionId, {
      userAgent: req && req.headers ? req.headers['user-agent'] : '',
      remoteAddress: req && req.socket ? req.socket.remoteAddress : ''
    });
    return sendJson(res, 200, { ok: true, claimedAt: lock.claimedAt, lastSeenAt: lock.lastSeenAt });
  } catch (err) {
    return sendJson(res, err && err.status ? err.status : 500, {
      ok: false,
...
async function handleSiteLockHeartbeat(req, res) {
  try {
    const body = await readJsonBody(req);
    const lock = refreshSiteLock(body && body.fingerprint, body && body.sessionId, {
      userAgent: req && req.headers ? req.headers['user-agent'] : '',
      remoteAddress: req && req.socket ? req.socket.remoteAddress : ''
    });
    return sendJson(res, 200, { ok: true, lastSeenAt: lock.lastSeenAt });
  } catch (err) {
    return sendJson(res, err && err.status ? err.status : 500, {
      ok: false,
      error: err && err.message ? err.message : 'internal error',
...
async function handleSiteLockRelease(req, res) {
  try {
    const body = await readJsonBody(req);
    const released = releaseSiteLock(body && body.fingerprint, body && body.sessionId);
    return sendJson(res, 200, { ok: true, released: !!released });
  } catch (err) {
    return sendJson(res, err && err.status ? err.status : 500, {
      ok: false,
      error: err && err.message ? err.message : 'internal error',
...
setInterval(pruneExpiredSiteLocks, 5 * 1000).unref();

function setInitialTurnForRoom(room, firstPlayerIndex) {
  if (!room) return;
  const nextTurnIndex = firstPlayerIndex === 1 ? 1 : 0;
  room.turnIndex = nextTurnIndex;
  const state = room.state || {};
  if (state.axis4) state.axis4.turn = nextTurnIndex + 1;
  if (state.gomoku) state.gomoku.turn = nextTurnIndex + 1;
  if (state.dab) state.dab.turn = nextTurnIndex + 1;
  if (state.quoridor) state.quoridor.current = nextTurnIndex;
  if (state.blokus) state.blokus.current = nextTurnIndex;
  if (state.orbito) state.orbito.current = nextTurnIndex + 1;
  if (state.halma) state.halma.current = nextTurnIndex;
}

function normalizePlayerStreak(value) {
...
function getRoomPlayerNames(room) {
  ...
  return room.players.map((pid, index) => {
    ...
    return {
      ...
      playerStreak: player ? normalizePlayerStreak(player.playerStreak) : 0
    };
  });
}
  ...
  for (let pieceIndex = 0; pieceIndex < BLOKUS_PIECES.length; pieceIndex += 1) {
    ...
    for (let flip = 0; flip < 2; flip += 1) {
      for (let rot = 0; rot < 4; rot += 1) {
        ...
        const key = shape.map((c) => c[0] + ',' + c[1]).join(';');
...
function applyOrbitoMove(state, payload, playerIndex) {
  ...
  if (action === 'move_enemy') {
    ...
    if (lines.red && lines.blue) {
      ...
      // Orbito's published rules use five extra button presses after the last marble is placed.
...
function maybeStartRoom(roomId) {
  ...
  room.players.forEach((pid) => {
    ...
    sendEvent(player, 'start', {
      ...
      playerStreak: normalizePlayerStreak(player.playerStreak),
...
async function handleJoin(req, res) {
  try {
    ...
    const playerId = uuid();
    const clientId = normalizeClientId(body.clientId) || uuid().replace(/-/g, '');
    const existingClientPlayer = findActivePlayerByClientId(clientId);
    if (existingClientPlayer) {
      const isQueuedWithoutSse = existingClientPlayer.status === 'queued' && !existingClientPlayer.res;
      const staleQueuedJoin = isQueuedWithoutSse && (now() - (Number(existingClientPlayer.joinedAt) || 0) >= QUEUED_PLAYER_CONNECT_GRACE_MS);
      // A queued entry with no SSE for a few seconds is usually a stale join attempt
      // from the same browser profile. Replace only those stale entries so a fresh
      // join cannot be accidentally stolen during the brief connect window.
      if (staleQueuedJoin) {
        cleanupPlayer(existingClientPlayer.playerId, 'player_left');
      } else {
        return sendJson(res, 409, {
          error: '같은 브라우저에서는 한 번에 한 창만 온라인 플레이를 할 수 있어.',
          code: 'client_already_online'
        });
      }
    }
    const rawRequestedPlayerName = Object.prototype.hasOwnProperty.call(body, 'playerName')
      ? body.playerName
      : (getReservedPlayerNameForClient(clientId) || '');
    const requestedPlayerName = validateRequestedPlayerName(rawRequestedPlayerName);
    const playerName = reservePlayerNameForClient(requestedPlayerName, clientId);
    const playerStreak = normalizePlayerStreak(body.playerStreak);
    const player = {
      playerId,
      clientId,
      playerName,
    ...
    while (queue.length > 0) {
      ...
      roomId = uuid();
      const playerSlots = Math.random() < 0.5 ? [otherPlayer.playerId, playerId] : [playerId, otherPlayer.playerId];
      rooms[roomId] = {
        roomId,
        gameType,
        players: playerSlots,
      ...
      setInitialTurnForRoom(rooms[roomId], Math.random() < 0.5 ? 0 : 1);

      otherPlayer.roomId = roomId;
      otherPlayer.playerIndex = playerSlots.indexOf(otherPlayer.playerId);
      otherPlayer.status = 'matched';
      otherPlayer.matchedAt = now();

      player.roomId = roomId;
      player.playerIndex = playerSlots.indexOf(player.playerId);
...
async function handleMove(req, res) {
  try {
    ...
    if (room.gameType === 'dab') {
      ...
      keepTurn = !!gomokuResult.keepTurn;
...
async function handleChat(req, res) {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(body, 'playerStreak')) {
      player.playerStreak = normalizePlayerStreak(body.playerStreak);
    }
...
async function handleProfile(req, res) {
  try {
    ...
    const player = requestedPlayerId ? players[requestedPlayerId] : null;
    if (requestedPlayerId && !player) return sendJson(res, 400, { error: 'invalid player' });

    const clientId = normalizeClientId(body.clientId) || (player && normalizeClientId(player.clientId)) || uuid().replace(/-/g, '');
    let resolvedName = player ? player.playerName : (getReservedPlayerNameForClient(clientId) || '');
    let resolvedStreak = player ? normalizePlayerStreak(player.playerStreak) : normalizePlayerStreak(body.playerStreak);

    const activePlayerForClient = findActivePlayerByClientId(clientId, player ? player.playerId : null);
    if (!player && activePlayerForClient && Object.prototype.hasOwnProperty.call(body, 'playerName')) {
      const requestedOfflineName = normalizePlayerName(body.playerName);
      if (canonicalizePlayerName(requestedOfflineName) !== canonicalizePlayerName(activePlayerForClient.playerName)) {
        return sendJson(res, 409, {
          error: '같은 브라우저에서 이미 온라인 플레이 중이라서 닉네임을 바꿀 수 없어.',
          code: 'name_change_only_in_menu'
        });
      }
    }

    if (Object.prototype.hasOwnProperty.call(body, 'playerName')) {
      const requestedName = validateRequestedPlayerName(body.playerName);
      if (player && isPlayerNameActive(player) && canonicalizePlayerName(requestedName) !== canonicalizePlayerName(player.playerName)) {
        return sendJson(res, 409, {
          error: '닉네임 변경은 메인 화면에서만 할 수 있어.',
          code: 'name_change_only_in_menu'
        });
      }
      resolvedName = reservePlayerNameForClient(requestedName, clientId, player ? player.playerId : null);
      if (player) player.playerName = resolvedName;
    }
    ...
    if (player) {
      ...
      resolvedStreak = normalizePlayerStreak(player.playerStreak);
    }
    ...
    if (player && player.roomId && rooms[player.roomId]) {
      ...
      broadcast(room.roomId, 'profile', payload);
      broadcast(room.roomId, 'state_sync', {
        roomId: room.roomId,
        players: getRoomPlayerNames(room),
        snapshot: getRoomSnapshot(room)
      });
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && routeMatches(pathname, '/site-lock-claim')) return void handleSiteLockClaim(req, res);
  if (req.method === 'POST' && routeMatches(pathname, '/site-lock-heartbeat')) return void handleSiteLockHeartbeat(req, res);
  if (req.method === 'POST' && routeMatches(pathname, '/site-lock-release')) return void handleSiteLockRelease(req, res);
  ...
  if (req.method === 'POST' && routeMatches(pathname, '/profile')) return void handleProfile(req, res);
