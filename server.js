...
function createAirHockeyState() {
  return {
    ...
    puck: { x: AIR_W / 2, y: AIR_H / 2, vx: 0, vy: 0 },
    scores: [0, 0],
    phase: 'waiting',
    serveAt: 0,
...
function airCollidePaddle(state, p) {
  ...
  // 이미 서로 멀어지는 중인데 겹쳐 있기만 한 경우에는 위치만 밀어내고 속도는 뒤집지 않는다.
  // 이 처리가 없으면 패들이 퍽에 붙어 있을 때 퍽이 떨리거나 과하게 튈 수 있다.
  if (relativeNormal >= 0) return;

  let outNormal = Math.max(-puckNormal + paddleNormal * (1 + AIR_HIT_BOOST), AIR_MIN_SHOT_SPEED);
  outNormal += Math.min(4.5, Math.abs(relativeNormal) * 0.35);
  const outTangent = puckTangent * 0.55 + paddleTangent * 0.85;
...
function airStepRoom(room) {
  ...
  const puck = state.puck;
  puck.x += puck.vx * dtRatio;
  puck.y += puck.vy * dtRatio;
  ...
  if (puck.x - AIR_PUCK_R < 0) { puck.x = AIR_PUCK_R; puck.vx = Math.abs(puck.vx); }
  if (puck.x + AIR_PUCK_R > AIR_W) { puck.x = AIR_W - AIR_PUCK_R; puck.vx = -Math.abs(puck.vx); }

  let scorer = -1;
  if (puck.y - AIR_PUCK_R <= 0) {
    if (puck.x >= leftGoal && puck.x <= rightGoal) scorer = 1;
    else { puck.y = AIR_PUCK_R; puck.vy = Math.abs(puck.vy); }
  }
  if (puck.y + AIR_PUCK_R >= AIR_H) {
    if (puck.x >= leftGoal && puck.x <= rightGoal) scorer = 0;
    else { puck.y = AIR_H - AIR_PUCK_R; puck.vy = -Math.abs(puck.vy); }
  }
